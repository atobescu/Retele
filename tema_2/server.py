...
while True:
    try:
        ...
        if comanda == 'CONNECT':
            ...
            raspuns = "EROARE: Trebuie sa fii conectat (CONNECT) pentru a folosi aceasta comanda."
            ...
            if not argumente:
                ...
                mesaje_postate.append(nou_mesaj)
            ...
            if not argumente.isdigit():
                ...
                target_id = int(argumente)
                gasit = False
                for i, m in enumerate(mesaje_postate):
                    if m['id'] == target_id:
                        gasit = True
                        if m['autor'] == adresa_client:
                            mesaje_postate.pop(i)
                            raspuns = f"OK: Mesajul {target_id} a fost sters."
                        else:
                            raspuns = "EROARE: Nu poti sterge mesajele altora!"
                        break

                if not gasit:
                    raspuns = f"EROARE: Mesajul cu ID={target_id} nu exista."

        elif comanda == 'LIST':
            if not mesaje_postate:
                raspuns = "INFO: Nu exista mesaje publicate."
            else:
                linii = [f"ID {m['id']} [{m['autor']}]: {m['text']}" for m in mesaje_postate]
                raspuns = "\n" + "\n".join(linii)
