...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE LOCALA: Trebuie sa te conectezi mai intai folosind comanda CONNECT.")
            continue

        if comanda == 'PUBLISH' and not argumente:
            print("EROARE LOCALA: Comanda PUBLISH necesita un text (ex: PUBLISH salut).")
            continue

        if comanda == 'DELETE' and not argumente.isdigit():
